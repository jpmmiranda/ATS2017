...
public class Login {
        ...
        do {
            ...
            if (i == 1) {
                ...
                if (nLugares > 0 && nLugares < 10) {
                    chamarViatura(c, utilizadores, nLugares);
                } else {
    ...
    private void chamarViatura(Cliente c, Utilizadores utilizadores, int nLugares) {
        ...
        do {
            ...
            if (!sucesso) {
                System.out.println("Erro nas coordenadas!!!");
            }
        } while (!sucesso);
        ...
        if (emailmotorista == null) {
            ...
            try {
                ...
                System.out.println("Unexpected Error. Estrutura corrompida?");
            }
        }
        ...
        do {
            ...
            if (i == 1) {
                ...
                if (m.getVeiculo() != null) {
                    System.out.println(m.getVeiculo().toString());
                }
            } else if (i == 6) {
    ...
    private void verHistorico(Ator a) {
        ...
        if (op == 1) {
            ...
            if (historico.size() > 0) {
                ...
                System.out.println("O utilizador não realizou nenhuma viagem até ao momento.");
            }
        } else if (op == 2) {
            while (cond) {
                ...
                try {
                    ...
                    System.out.println("\n-----------Historico de viagens-----------");
                    if (hist.size() > 0) {
                        hist.forEach(v -> System.out.println(v.toString()));
                    } else {
                        System.out.println("Não existem registos de viagem no período considerado.");
                    }
                    cond = false;
                } catch (InvalidIntervalException e) {
                    System.out.println("Certefique-se de que a data de inicio é mais antiga que a de fim.");
                    cond = true;
                }
            }
        } else {
    ...
    public void realizarViagem(Cliente c, Motorista m, double distanciaAteCliente, Coordenada inicio, Coordenada fim) {
        ...
        /**
         * Garantimos que a fiabilidade não passa de 100
         */

        double tempoReal = tempoTotalEsperado + (1 - fiab) * tempoTotalEsperado;

        /**
         * Por consequencia de que, pra já, tempo real = tempo esperado, o preço
         * a pagar vai se calcular para a diferença entre tempo <25%.
         */
        ...
        if (diferenca > 0.25 * tempoTotalEsperado) {
            sb.append("Desculpe pelo atraso, terá um desconto de ")
                    .append(String.format("%.2f", 50 * diferenca / tempoTotalEsperado))
                    .append("%.");
            System.out.println(sb);
            preco = preco * (diferenca / tempoTotalEsperado);
        }
        sb.setLength(0);
        sb.append("Preço a pagar: ").append(sb).append(String.format("%.2f", preco)).append("€");
        System.out.println(sb);
        /**
         * Atualização do dinheiro total investido no sistema UMeR por este
         * cliente.
         */
    ...
    public GregorianCalendar dataInicial() {
        ...
        while (cond) {
            ...
            if (diaI > 0 && diaI < 29
                    ...
                    || diaI == 29 && aux.isLeapYear(anoI)) {
                cond = false;
            } else {
                System.out.println("Introduza um dia válido!");
            }
        }
